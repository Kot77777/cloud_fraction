...
namespace cloud_fraction {
    ...
    //Функция находит нижнюю границу окрестноти. Верхняя очевидно down_date_ + n_.
    double Dynamic_model::time_bound(const double t) const {
        ...
        // Тут вычисляется верзняя граница времени, в которцю относительно времени t влезает целое число step_t_.
        const double max_int_time = t + std::floor((max_time - t) / step_t_) * step_t_;

        // Тут вычисляется нижняя граница (учитывая все случаи(вроде)) так, чтобы по итогу полное время моделирования было n_.
    ...
    double Dynamic_model::process(const double t, const double width, const double len) {
        ...
        //Вычисляем остальные значения. Тут так i / step_ind, потому что нужно целое число после деления.
